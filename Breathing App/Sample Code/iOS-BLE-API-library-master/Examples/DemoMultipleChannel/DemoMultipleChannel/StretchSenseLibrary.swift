 ...
 **Definitions:**
 - **Peripheral**: Bluetooth 4.0 enabled sensorsa
...
class StretchSenseAPI: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        ...
        for myPeripheralAvailable in listPeripheralAvailable{
            if (peripheral.identifier.uuidString == myPeripheralAvailable?.identifier.uuidString){
                let indexPositionToDelete = listPeripheralAvailable.index(where: { $0?.identifier.uuidString == (myPeripheralAvailable?.identifier.uuidString)!  }) // looking for the index of the peripheral to delete in the list
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        for charateristic in service.characteristics! {
            ...
            /*if thisCharacteristic.uuid.uuidString == samplingTimeUUID2.uuidString{
             print(("writeValue serviceUUID2"))
             
             var dataIn = 0;
             var dataUint8: UInt8 = UInt8(dataIn)
             let dataNSData = NSData(bytes: &dataUint8, length: 1)
             print(("writeValue"))
             peripheral.writeValue(dataNSData as Data, for: thisCharacteristic, type: CBCharacteristicWriteType.withResponse)
             }*/
