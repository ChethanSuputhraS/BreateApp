...
class LineChartViewController: UIViewController, UIPopoverPresentationControllerDelegate, UITableViewDataSource, UITableViewDelegate  {
    ...
    override func viewDidLoad() {
        ...
        lineChart.autoMinMax = true // by default, the lineChart auto rescale, if we press a "scale button" the auto stop
    ...
    func ask_new_data(){
        ...
        listPeripheralsConnected = stretchsenseObject.getListPeripheralsConnected()
        //print(listPeripheralsConnected)
        //newConnection = false
        reloadGraph()
        tableValue.reloadData()
    ...
    func reloadGraph(){
        ...
        if listPeripheralsConnected.count != 0 {
            ...
            for myPeripheralConnected in listPeripheralsConnected {
                ...
                //print("myPeripheralConnected.previousValueAveraged")
                if myPeripheralConnected.display == true{

                    lineChart.addLine(myPeripheralConnected.previousValueAveraged, colorNumber: myPeripheralConnected.uniqueNumber)
                }
            }
        }
