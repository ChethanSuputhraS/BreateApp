...
class NewLineCharts: UIView {
    
    var isViewSett = false
...
//            drawLables()
     ...
     func drawCurvedChartwith(dataEnt : [PointEntry])
    {
        if isViewSett == false
        {
            self.setNeedsLayout()
        ...
        dataPoints = convertDataEntriesToPoints(entries: dataEnt)
    ...
    func drawChartWith(dataEnt : [PointEntry])
    {
        for lineLayer in lineLayerStore {
