...
//            peripheral.discoverServices(nil)
        ...
        func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
            ...
            if isAllowScan == true
            {
//                print("ValueData",capacitance_value_in_pf,myString)
                didUpdateValueGenTako(peripheral, characteristic: characteristic)
                isAllowScan = false

            }
//            let defaultCenter0 = NotificationCenter.default
//            defaultCenter0.post(name: Notification.Name(rawValue: "kpRecordMethod"), object: capacitance_value_in_pf, userInfo: nil)
        ...
        func didUpdateValueGenTako(_ peripheral: CBPeripheral, characteristic: CBCharacteristic){
            ...
            let nNotificationToSend = 1+((nSensor-1)/10)
            ...
            innerLoop: for myPeripheral in listPeripheralConnectedTako
            {
                if myPeripheral.uuid == peripheral.identifier.uuidString
                {
                    ...
                    var j = 0;
                    var nSensorInCaracteristic = nSensor - offset*1
                    if nSensorInCaracteristic > 1
                    {
                    ...
                    for i in 0...(nSensorInCaracteristic)-1
                    {
                        ...
                        let valueIntSense1:Int! = Int(valueRawSensor1.hexadecimalString()!, radix: 16)!
                        var capacitance_value_in_pf = ( CGFloat(valueIntSense1)/10.0 ) * 1.0//KPMarch030320
                        if capacitance_value_in_pf > 340
                        {
//                            capacitance_value_in_pf = 340
                        }
//                        capacitance_value_in_pf = capacitance_value_in_pf * 1.5
                        ...
                        {
//
//                            print("------->",capacitance_value_in_pf, peripheralToShift.previousValueAveraged[k],peripheralToShift.previousValueAveraged[k+1] )
                            ...
                            peripheralToShift.times[k] = peripheralToShift.times[k+1]

                        }
                        
//                        timesCounts = timesCounts + 100
//                        if timesCounts == 1100
//                        {
//                            timesCounts = 0
//                        }
...
//                        let myString = formatter.string(from: Date())
//
//                        let doubleValue = Double(myString)
//                        let mms  = timesCounts
//
//                        let lastVal : CGFloat = CGFloat(peripheralToShift.previousValueRawFromTheSensor[peripheralToShift.previousValueRawFromTheSensor.count-1])
//                        let lastTime : CGFloat = CGFloat(peripheralToShift.times[peripheralToShift.times.count-1])
////                        print("time",mms,lastTime, lastVal,capacitance_value_in_pf)
////                        print("time",peripheralToShift.previousValueAveraged,peripheralToShift.previousValueRawFromTheSensor, peripheralToShift.times,capacitance_value_in_pf)
//
//                        let upValues : CGFloat = CGFloat(capacitance_value_in_pf) - lastVal
//                        let downValues : CGFloat = CGFloat(mms - lastTime)
//                        var finalVal : CGFloat = CGFloat(upValues / downValues)
//                        print("ValueData",finalVal * 1000)
//                        finalVal = finalVal * 280
                        peripheralToShift = listPeripheralsConnected[indexOfperipheral!+i]
                        peripheralToShift.previousValueRawFromTheSensor[peripheralToShift.previousValueRawFromTheSensor.count-1] = capacitance_value_in_pf
                        peripheralToShift.previousValueAveraged[peripheralToShift.previousValueAveraged.count-1] = capacitance_value_in_pf
//                        peripheralToShift.times[peripheralToShift.times.count-1] = CGFloat(mms)
        ...
        var scanTimer = Timer()
        var isAllowScan : Bool = false

        func startBluetooth()
        {
            scanTimer = Timer.scheduledTimer(timeInterval: 0.2, target: self, selector: #selector(UpdateTimer), userInfo: nil, repeats: true)
