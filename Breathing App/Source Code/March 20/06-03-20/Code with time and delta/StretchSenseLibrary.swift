...
//            peripheral.discoverServices(nil)
        ...
        func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
            ...
            if isAllowScan == true
            {
//                timesCounts = timesCounts + 1
//                print("ValueData",capacitance_value_in_pf,myString,timesCounts)
            ...
            innerLoop: for myPeripheral in listPeripheralConnectedTako
            {
                if myPeripheral.uuid == peripheral.identifier.uuidString
                {
                    ...
                    for i in 0...(nSensorInCaracteristic)-1
                    {
                        ...
                        let myString = formatter.string(from: Date())
                        let numbers = Double (myString)
                        timesCounts = CGFloat(numbers ?? 0.0)
                        
                        if lastVal == 0
                        {
                            lastVal = capacitance_value_in_pf
                            lastTimeVal = timesCounts
                            finalValue = capacitance_value_in_pf
                        }
                        else
                        {
...
//                            }
                            ...
                            finalValue = (capacitance_value_in_pf - lastVal) / (timesCounts - lastTimeVal)
//                            finalValue = finalValue * 1000
//                            if finalValue > 10
//                            {
//                                return
//                            }
//                            print("FInal Val",capacitance_value_in_pf, lastVal, timesCounts, lastTimeVal,finalValue)

                            lastVal = capacitance_value_in_pf
                            lastTimeVal = timesCounts
                        }
                        capacitance_value_in_pf = finalValue
                        ...
                        if ( filtering_value >= 2)
                        {
                            ...
                            print("After Filter",capa_averaged_after_filtering,timesCounts)
                        }
                        
                        
//                        print("After Filter 1 ",peripheralToShift.previousValueRawFromTheSensor, peripheralToShift.previousValueRawFromTheSensor.count)
