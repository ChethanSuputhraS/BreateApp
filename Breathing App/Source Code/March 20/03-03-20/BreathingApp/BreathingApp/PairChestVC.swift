...
class PairChestVC: UIViewController,UITableViewDelegate,UITableViewDataSource
{
    ...
    override func viewDidLoad()
    {
        ...
        if (strChestUUID != "NA" && strAbdUUID != "NA")
        {
            isDevicesAssigned = true
        }

        self.SetupForCardView()
    ...
    @objc func SetupForCardView()
    {
        ...
        self.view.addSubview(imgTickAnimation)
        ...
        self.view.addSubview(btnPairing)
    ...
    @objc func CheckDeviceConnected()
    {
        ...
        if pageNumber == 1
        {
            if connectionSuccess == 1
            {
                ...
                UIView.transition(with: self.btnPairing, duration: 0.5, options: .curveEaseOut, animations: nil, completion: nil)
            ...
            {
                let alert = UIAlertController(title: "Breathimg APP", message: "Something went wrong. Please try again later.", preferredStyle: UIAlertController.Style.alert)
        ...
        else if pageNumber == 2
        {
            ...
            btnPairing.frame = CGRect(x: 30, y: constants.heighs-170, width: constants.widths-100, height: 65)

            if connectionSuccess == 1
            {
                self.imgTickAnimation.backgroundColor = UIColor.clear

                self.btnPairing.setTitle("Abdomen Strap Connected", for: .normal)
                UIView.transition(with: self.btnPairing, duration: 0.5, options: .curveEaseOut, animations: nil, completion: nil)
                self.imgTickAnimation.image = UIImage.init(named: "tick4.png")
                let Rvc = RecordVC()
            ...
            {
                self.imgTickAnimation.frame = CGRect(x: 30, y: constants.heighs-170, width: constants.widths-100, height: 65)
    ...
    {
        self.perform(#selector(self.showAbdomen), with: nil, afterDelay: 0.5)
    }
    ...
    @objc func showAbdomen()
    {
        UIView.animate(withDuration: Double(0.5), animations: {
            ...
            UIView.transition(with: self.bodyView, duration: 0.5, options: .transitionFlipFromRight, animations: nil, completion: nil)
            self.tickTimer.invalidate()
        })
    }
    @objc func update()
...
//            self.btnRefrshClick()
            ...
            if listPeripheralsAvailable.count == 0
            {
                //Alert popup for not having any devices
            }
            else
            {
                ACProgressHUD.shared.hideHUD()
                ...
                if pageNumber == 1
                {
                    self.btnPairing.setTitle("Pairing Chest Strap", for: .normal)

                    let strChestUUID: NSString = UserDefaults.standard.value(forKey: "ChestUUID") as? NSString ?? "NA"
                    strSelectedPeripheral = strChestUUID
                    stretchsenseObject.connectToPeripheralWithUUID(strChestUUID as String)
                }
                else if pageNumber == 2
                {
                    self.btnPairing.setTitle("Pairing Abdomen Strap", for: .normal)

                    let strChestUUID: NSString = UserDefaults.standard.value(forKey: "AbdomenUUID") as? NSString ?? "NA"
                    strSelectedPeripheral = strChestUUID
                    stretchsenseObject.connectToPeripheralWithUUID(strChestUUID as String)
                }
                ...
                self.perform(#selector(self.CheckDeviceConnected), with: nil, afterDelay: 3)
            }

        }
        else
        {
            let alert = UIAlertController(title: "Breathimg APP", message: "You have not assigned Strap Type \n Please assign first", preferredStyle: UIAlertController.Style.alert)
            alert.addAction(UIAlertAction(title: "OK", style: UIAlertAction.Style.default)
            {
                ACTION -> Void in
...
//        headerView.backgroundColor = UIColor.white
        ...
        lbltitle.text = "  Please assign Strap Type to devices"
   ...
   func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int
   {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        if listPeripheralsAvailable.count != 0
        {
            ...
            {
                cell.lblBorder.backgroundColor = UIColor.init(red: 106.0/255, green: 227.0/255, blue: 255.0/255, alpha: 1)
                cell.lblBorderAbdomen.backgroundColor = UIColor.clear
            }
            else if strAbdUUID == strUUID
            {
                cell.lblBorderAbdomen.backgroundColor = UIColor.init(red: 0.97, green: 0.74, blue: 0.22, alpha: 1)
                cell.lblBorder.backgroundColor = UIColor.clear
            }
            cell.btnChest.tag = indexPath.row
            cell.btnAbdomen.tag = indexPath.row
            
            print("UUID Chest", strUUID, strChestUUID)
            
            cell.lblName.text = "StretchSense"
//            cell.lblAddres.text = "123123123123123123123123123123123123"
            cell.selectionStyle = .none
            tableView.separatorStyle = .none
            cell.backgroundColor = UIColor.clear
            cell.btnChest.addTarget(self, action: #selector(btnChestClick), for: .touchUpInside)
            cell.btnAbdomen.addTarget(self, action: #selector(btnAbdomenClick), for: .touchUpInside)
        }
        return cell
    }
...
//    css code
    @objc func btnChestClick(_ sender: AnyObject)
    {
        var arrDevices = stretchsenseObject.getListPeripheralsAvailable()
        if arrDevices.count > sender.tag
        {
            isDevicesAssigned = true
            
            let strUUID : NSString = (arrDevices[sender.tag]!.identifier.uuidString as NSString)
            UserDefaults.standard.set(strUUID, forKey: "ChestUUID")
            UserDefaults.standard.synchronize()
        }
        let strChestUUID: NSString = UserDefaults.standard.value(forKey: "ChestUUID") as? NSString ?? "NA"

        for i in 0...arrDevices.count-1
        {
            let strUUID : NSString = (arrDevices[i]!.identifier.uuidString as NSString)
    ...
    @objc func btnAbdomenClick(_ sender: AnyObject)
    {
        var arrDevices = stretchsenseObject.getListPeripheralsAvailable()
        if arrDevices.count > sender.tag
        {
