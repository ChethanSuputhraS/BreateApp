...
struct constants
{
    ...
    static let IS_IPHONE_6plus = UIScreen .main.bounds.size.height == 736.0
    static let IS_IPHONE_X = UIScreen .main.bounds.size.height == 812.0 || UIScreen .main.bounds.size.height == 896.0
    static let IS_IPHONE_11_Pro = UIScreen.main.bounds.size.height == 895.0
...
class LogInVC: UIViewController,UITextFieldDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        lblLoginTitle.frame = CGRect(x: 0, y: ha, width: self.view.frame.width, height: 44)
        ...
        var hb = CGFloat(constants.heighs-550)
        if (constants.IS_IPHONE_X)
        {
            hb = hb - 20
        }
        ...
        lblBreathe.frame = CGRect(x: 0, y: hb, width: constants.widths, height: 100)
        ...
        setUpTextField(txtField: txtLogInUserName, placeHolderText: "Name", yAxis: Int(constants.heighs-450), boardType: .emailAddress, capitalization: .none, imgeName: "User.png")
       LoginView.addSubview(txtLogInUserName)
        ...
        var hr = CGFloat(constants.heighs-325)
        if (constants.IS_IPHONE_X)
        {
            hr = hr - 40
        }
        
        btnRemeberMe.frame = CGRect(x: 35, y: hr, width: 200, height: 50)
  ...
  //MARK: - Sign up View
        ...
        lblSignUp.frame = CGRect(x: 0, y: hs, width: self.view.frame.width, height: 44)
        ...
        signUpView.addSubview(lblSignUp)
         ...
         setUpTextField(txtField: txtUserName, placeHolderText: "Name", yAxis: Int(constants.heighs-550), boardType: .emailAddress, capitalization: .none, imgeName: "User.png")
        txtUserName.returnKeyType = .next
        signUpView.addSubview(txtUserName)
        
         setUpTextField(txtField: txtEmailSignUp, placeHolderText: "E mail", yAxis: Int(constants.heighs-490), boardType: .emailAddress, capitalization: .none, imgeName: "User.png")
        ...
        var  ht = CGFloat(constants.heighs-320)
             if (constants.IS_IPHONE_X)
             {
                 ht = ht - 40
             }
        
        btnTermsAdnCondition.frame = CGRect(x: 35, y: ht, width: constants.widths, height: 50)
...
// MARK: - TextFiledMethod
    ...
    @objc func setUpTextField(txtField : UITextField , placeHolderText : String , yAxis : Int , boardType : UIKeyboardType , capitalization : UITextAutocapitalizationType ,imgeName : String)
    {
        ...
        txtField.frame = CGRect(x: 20, y: Int(yAxish), width: Int((constants.widths)-40), height: 50)
...
func textFieldShouldReturn(_ textField: UITextField) -> Bool
   ...
   @objc func btnSignInClick()
   {
        ...
        {
            let alert = UIAlertController(title: "Breathimg APP", message: "E mail", preferredStyle: UIAlertController.Style.alert)
        ...
        if (!(appDelegate?.checkForValidString(String : (appDelegate?.globalDeviceToken)!).isEqual(to: "NA"))!)
            ...
            progressView.progressText = "Logging..."
                    ...
                    case .success(let success):
                        ...
                        if let result = response.result.value
                        {
                            ...
                            if (strMsG as AnyObject).isEqual(to: "Email not verified!!, Plase verify your email address.")
                            {
                                ...
                                let alert = UIAlertController(title: "Breathing", message: "Please check your Email account and \"verify\" to log in.", preferredStyle: UIAlertController.Style.alert)
                                alert.addAction(UIAlertAction(title: "OK", style: UIAlertAction.Style.default)
                                {
                                    ACTION -> Void in
                                })
                            ...
                            {
                                let alert = UIAlertController(title: "BreathingApp", message: " Wrong Password...!", preferredStyle: UIAlertController.Style.alert)
                                alert.addAction(UIAlertAction(title: "OK", style: UIAlertAction.Style.default)
                                {
                                    ACTION -> Void in
                                })
                            ...
                            else if (strMsG as AnyObject).isEqual(to: "You are logged in successfully.")
                            {
                                ...
                                let alert = UIAlertController(title: "BreathingAPP", message: "Logged in successfully", preferredStyle: UIAlertController.Style.alert)
                                alert.addAction(UIAlertAction(title: "OK", style: UIAlertAction.Style.default)
                                {
                                    ACTION -> Void in
                                    // home vc
                            ...
                            windowglobal?.layer.add(transition, forKey: kCATransition)
               windowglobal?.rootViewController = UINavigationController(rootViewController: homevc)
//                                    windowglobal?.rootViewController = UINavigationController(rootViewController: homevc)
...
//
                                       ...
                                       else if (strMSG as AnyObject).isEqual(to: "Thanks! You have successfully signed up")
                                       {
                                           ...
                                           let alert = UIAlertController(title: "Breathing App", message: "WellCome To BreathingApp", preferredStyle: UIAlertController.Style.alert)
                                           alert.addAction(UIAlertAction(title: "OK", style: UIAlertAction.Style.default)
                                           {
                                               ACTION -> Void in
                                           })
