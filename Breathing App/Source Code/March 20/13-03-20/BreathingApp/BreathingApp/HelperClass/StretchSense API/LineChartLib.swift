...
class LineChart: UIView {
    ...
    var autoMinMax = true
    var max: CGFloat = 294.0
    var min: CGFloat = 291.0
    
    var maxGlobe: CGFloat = 310.0
    var minGlobe: CGFloat = 280.0
    ...
    var colors: [UIColor] = [
        UIColor(red: 0.33, green: 0.82, blue: 0.98, alpha: 1),
        UIColor(red: 0.97, green: 0.74, blue: 0.22, alpha: 1),
    ...
    func getMidPointsforPoints(p1 : CGPoint, p2 : CGPoint) -> CGPoint
    {
        let op1 = (p1.x + p2.x) / 2
        let op2 = (p1.y + p2.y) / 2
        let outPoint : CGPoint =  CGPoint(x: op1, y :op2)
        return outPoint
    }
    func controlPointForPoints(p1 : CGPoint, p2 : CGPoint) -> CGPoint
    {
        var controlPoint : CGPoint = self.getMidPointsforPoints(p1: p1, p2: p2)
    ...
    fileprivate func drawLineKP(_ line : Line)
    {
        if line.dataStore.count != 0
        {
            ...
            var p1 : CGPoint = CGPoint(x: xValue, y: yValue)
            path.move(to: p1)
            ...
            for index in 1..<data.count-1
            {
                ...
                let p2 : CGPoint = CGPoint(x: xValue, y: yValue)
                let midPoint = self.getMidPointsforPoints(p1: p1, p2: p2)
                path.addQuadCurve(to: midPoint, controlPoint: self.controlPointForPoints(p1: midPoint, p2: p1))
                path.addQuadCurve(to: p2, controlPoint: self.controlPointForPoints(p1: midPoint, p2: p2))
                p1 = p2
                xx = xValue
                yy = yValue
            }
    ...
    fileprivate func drawLine(_ line: Line) {
        ...
        if line.dataStore.count != 0 {
            ...
            for index in 1..<data.count-1 {///////////////-1
                xValue = self.x.scale(CGFloat(index)) + x.axis.inset
                yValue = self.bounds.height - self.y.scale((data[index])) - y.axis.inset
                print(" \(yValue) \(data[index]) \(data[index] * 1.5)")
                //                yValue = yValue * 1.5
    ...
    func addLine(_ data: [CGFloat], colorNumber: Int) {
        ...
        let tempMax : CGFloat = data.max() ?? 0.0
        let tempMin : CGFloat = data.min() ?? 0.0
        //        if  tempMax > maxGlobe
        //        {
        //            max = tempMax
        //            min = tempMin
        ...
        //        max = data.max() ?? 0.0
        //        min = data.min() ?? 0.0
        var total : CGFloat = tempMax - tempMin
        //        print("Total=",total)
