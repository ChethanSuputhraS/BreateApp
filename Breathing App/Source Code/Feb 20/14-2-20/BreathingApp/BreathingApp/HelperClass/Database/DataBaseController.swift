...
class DataBaseController: NSObject
{
    ...
    @objc func createDB()
    {
        ...
        //u can create more tables here
        if sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS Readings (id INTEGER PRIMARY KEY AUTOINCREMENT, session TEXT , UUID TEXT , deviceID TEXT , deviceType TEXT , readings TEXT , status TEXT )", nil, nil, nil) != SQLITE_OK {
            let errmsg = String(cString: sqlite3_errmsg(db)!)
            print("error creating table: \(errmsg)")
        }
...
@objc func insertIntoTabledevicesDB(name: NSString , userID: NSString , deviceName:NSString , deviceType:NSString , UUID:NSString, isActive:NSString)
 ...
 // DB  User DATA
    func getTableUserDataFromDB() -> [[String:String]]
    {
        ...
        let queryString = "SELECT * FROM UserData"
