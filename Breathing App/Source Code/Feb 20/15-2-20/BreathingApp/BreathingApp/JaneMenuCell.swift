...
class JaneMenuCell: UITableViewCell
{
   ...
   let lblLine = UILabel()
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?)
    {
        ...
        lblMenuLable.frame = CGRect(x: 10, y: 0, width: GlobalVariables.Device_Width, height: 50)
        lblMenuLable.textColor = UIColor.white
        self.contentView.addSubview(lblMenuLable)
        
        lblLine.frame = CGRect(x: 0, y: 51, width: GlobalVariables.Device_Width, height: 2)
        lblLine.backgroundColor =  UIColor.init(red: 5.0/255, green: 56.0/255, blue: 83.0/255, alpha: 1)
        self.contentView.addSubview(lblLine)
        
        imgViewArrow.frame = CGRect(x: GlobalVariables.Device_Width-90, y: 5, width: 25, height: 27)
