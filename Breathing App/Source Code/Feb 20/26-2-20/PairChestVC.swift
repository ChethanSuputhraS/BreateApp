...
//            backTblView.addSubview(lblNoDevices)
    ...
    @objc func CheckDeviceConnected()
    {
        ...
        if isViewNumber == 2
        {
            ...
            UIView.transition(with: self.btnPairing, duration: 1, options: .curveEaseOut, animations: nil, completion: nil)
            self.imgTickAnimation.image = UIImage.init(named: "tick4.png")
            self.perform(#selector(self.askforAbdomen), with: nil, afterDelay: 0.5)
        }
        if isViewNumber == 3
        {
...
//    chethan commented for TableView Buttons
...
//    css code
    @objc func btnChestClick()
    {
        
       
    }
    @objc func btnAbdomenClick()
    {
